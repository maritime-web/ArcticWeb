(function () {
    ...
    function iceController($scope, IceService, $timeout, ShapeService, SubscriptionService, NotifyService, IceEvents) {
        NotifyService.notify(IceEvents.IceFeatureActive);
        $scope.selected = {};
        $scope.satellite = {};
            ...
            inshoreIceReport: {
                ...
                success: function (inshoreIceReport) {
                    ...
                    ShapeService.staticShapes("static.gre-inshore-icereport", {
                        ...
                        NotifyService.notify(IceEvents.ShowInshoreReports, shapes);
                    }, function (error) {
                    });
                }
            }
        ...
        NotifyService.subscribe($scope, IceEvents.ObservationSelected, function (e, ice) {
            if (!ice) {
        ...
        $scope.showInshore = function ($event, location) {
            $event.preventDefault();
            NotifyService.notify(IceEvents.ZoomToReport, location.number)
        };
        ...
        $scope.$on("$destroy", function () {
            NotifyService.notify(IceEvents.IceFeatureInActive);

            // inshoreLayer.clear();
        });
    }

    module.controller("IceController", [ '$scope', 'IceService', '$timeout', 'ShapeService', 'SubscriptionService', 'NotifyService', 'IceEvents', iceController ]);
