(function () {
    ...
    var metocModule = angular.module('embryo.weather');

    metocModule.factory('MetocService', MetocService);
    MetocService.$inject = ['$http', 'CookieService'];

    function MetocService($http, CookieService) {
            ...
            listMetoc: function (routeIds, callback, error) {
                ...
                $http.get(url, {
                    ...
                    .catch(function (response) {
                        embryo.messagePanel.replace(messageId, {
                            text: "Failed loading metoc. Server returned error: " + response.status,
                            type: "error"
                        });
                        if (error) {
                            error();
